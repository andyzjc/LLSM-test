function grapthLattice(NA1,deltaNA,LatticeType,weighting,LatticePSF,LatticePSFDithered,LatticePupil,PSFdet,savingdir)
    ...
    fig = figure;
    imagesc(KX_exc,KZ_exc,abs(DitheredxzOTFexc)/max(abs(DitheredxzOTFexc),[],'all'))
    axis image
    xlabel("k_x/(4\pin/\lambda_{exc})");
    ylabel("k_z/(4\pin/\lambda_{exc})");
    xlim([-0.5,0.5])
    ...
    clim([0,1])
    hold on
    plot(abs(DitheredzOTFexc)/max(abs(DitheredzOTFexc))/2,KZ_exc,'LineWidth',0.5,'Color','w')
    print(fig, '-dsvg', [savingdir   LatticeType '_' num2str(NA1) '_' num2str(deltaNA) '_absxzOTF_weighting_' num2str(weighting) '.SVG'],'-r300')
    print(fig, '-dpng', [savingdir   LatticeType '_' num2str(NA1) '_' num2str(deltaNA) '_absxzOTF_weighting_' num2str(weighting) '.PNG'],'-r300')
    ...
    clim([0,1])
    hold on
    plot(abs(zOTFoverall)/max(abs(zOTFoverall)),KZ_exc,'LineWidth',0.5,'Color','w')
    print(fig9, '-dsvg', [savingdir   LatticeType '_' num2str(NA1) '_' num2str(deltaNA) '_absxzOTF_overall_weighting_' num2str(weighting) '.SVG'],'-r300')
    print(fig9, '-dpng', [savingdir   LatticeType '_' num2str(NA1) '_' num2str(deltaNA) '_absxzOTF_overall_weighting_' num2str(weighting) '.PNG'],'-r300')
