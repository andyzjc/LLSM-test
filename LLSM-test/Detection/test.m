...
% 
[kx,ky,kz] = meshgrid( (  -(N-1)/2 : (N-1)/2  ),(  -(N-1)/2 : (N-1)/2  ),(  -(N-1)/2 : (N-1)/2 ));

test_3d = kx.^2+ky.^2+kz.^2 <= k_wave.^2;
% test_3d = (kx.^2+ky.^2) <= k_det.^2 & (kx.^2+ky.^2+kz.^2<=k_wave.^2) ;
% test_3d2 = test_3d;
% test_3d2((k_wave.^2 - test_3d) <= 0) = 0;

test_3d( (k_det.^2 - (kx.^2+ky.^2) <= 0 ) ) = 0;

% % test_3d(kz<=0)=0;
% test2 = abs(fftshift(fftn(test_3d))).^2;
figure
subplot(1,2,1)
xzPSF = test2(:,(N+1)/2,:);
imagesc(X_det,Z_det,squeeze(xzPSF)')
axis image 

subplot(1,2,2)
imagesc(KX_det,KZ_det,abs(fftshift(fft2(squeeze(xzPSF)'))))
axis image
figure
subplot(1,2,1)
xyPSF = test2(:,:,(N+1)/2);
imagesc(X_det,Y_det,xyPSF)
axis image
subplot(1,2,2)
imagesc(KX_det,KY_det,abs(fftshift(fft2(squeeze(xyPSF)))))
axis image

%%
Pupil = (k_det).^2 > (kx_det.^2 + ky_det.^2);
% Pupil = exp( -(kx_det.^2 + ky_det.^2)/ ((k_det/2).^2) );
...
xzPSFdetection = squeeze(PSFdet(:,(N+1)/2,:))'; xzPSFdetection = xzPSFdetection/max(max(xzPSFdetection));
