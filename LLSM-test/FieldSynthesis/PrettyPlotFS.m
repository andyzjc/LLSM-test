function PrettyPlotFS(FSPupil,FSMask,FSMetaData,FSPSF,FScenter)

    getParameters; %modify image parameter here
    CalculatePhysics;
    
    NAmax = FSMetaData.NAmax;
    NAmin = FSMetaData.NAmin;
    MaskNAmax = FSMetaData.MaskNAmax;
    MaskNAmin = FSMetaData.MaskNAmin;
    
    FSPSFDithered = FSPSF;
    FSPupil = sum(FSPupil,3);

    xzPSFexc = FSPSF(:,:,(N+1)/2);
    ...
    xzPSFexc_dither = FSPSFDithered(:,:,(N+1)/2); 
    ...
    yzPSFexc = squeeze(FSPSF(:,(N+1)/2,:));
    yzPSFexc_dither = squeeze(FSPSFDithered(:,(N+1)/2,:));
...
%%
    ...
    subplot(1,2,1);
    image11 = imagesc(KX_exc,KZ_exc, real(FSPupil) );
    title("Rear Pupil, " +...
          "N_{xz} = " + num2str(N))
    ...
    Illum_mask = imfuse(real(FSPupil),FSMask,"falsecolor","ColorChannels","green-magenta");
    subplot(1,2,2)
